...
const User = require('../models/User'); // Assuming you have a User model for uploadedById
 ...
 * @description Add a document to a project (file upload)
 * @access Private
 */
router.post('/:id/documents', upload.single('file'), async (req, res) => {
  const { id: projectId } = req.params;
  const { name, type,  uploadedBy, hodOnly, dataAiHint } = req.body;

  // Basic validation for required fields
  if (!name || !type || !uploadedBy || !req.file) {
    return res.status(400).json({ message: 'Please provide name, type, uploadedBy, and a file.' });
  }
  ...
  try {
    ...
    const newDocument = {
      name,
      type,
      url: `/uploads/${req.file.filename}`, // File path or URL
      size: req.file.size,
      uploadedBy,
      hodOnly: hodOnly || false,
      dataAiHint,
      uploadedAt: new Date()
    };
    console.log('New document object (bknd) :', newDocument);

    project.documents.unshift(newDocument);
    await project.save();
    console.log('Document added to project:', newDocument);
    res.status(201).json(project.documents[0]);
  } catch (error) {
 ...
 * @access  Private (e.g., requires authentication/authorization)
 */
router.get('/:id/documents', async (req, res) => {
  const { id: projectId } = req.params;

  try {
    // Find the project by its ID and select only the 'documents' field
    const project = await StoreProject.findById(projectId).select('documents');
...
//   // ... other exported project controller functions
// };
